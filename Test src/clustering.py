...
def main():
	...
	time1s = time.time()
	photos = loadimages("sample_data4")			# load photos from folder	
	time1e = time.time()
	...
	busid = []
	busvectorim = []
	dataPhoto = preparePhotoData(photovector,busid,busvectorim,"test")
...
def getRestPhoto(kindofdataset):
	csv_file = open(""+kindofdataset+"_photo_to_biz.csv","rb")
